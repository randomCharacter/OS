...
public:
    ...
    void uputi_UI_zahtev(int id_procesa, int broj_staze) {
        ...
        // Pravi novi zahtev i dodaje u vektor
    ...
    int obradi_zahtev() {
        ...
        auto z = zahtevi.begin();
        int min = abs((*z)->broj_staze - staza);
        for (auto it = zahtevi.begin(); it != zahtevi.end(); it++) {
            if (abs((*it)->broj_staze - staza) < min) {
                min = abs((*it)->broj_staze - staza);
                z = it;
            }
        }
        // Vadi zahtev iz vektor
        staza = (*z)->broj_staze;
        // Izvršava zahtev
        l.unlock();
        this_thread::sleep_for(milliseconds(300));
        l.lock();
        // Završava zahtev i briše ga iz vektora
        (*z)->gotov = true;
        (*z)->cv.notify_one();
        zahtevi.erase(z);
        return staza;
